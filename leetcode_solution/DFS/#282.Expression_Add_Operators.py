...
class Solution:
    ...
    def DFS(self, num, target, ex, stack, result):
        ...
        for i in range(len(num)):
            ...
            self.DFS(num[i+1:], target, ex+"+"+num[:i+1], stack+["+", num[:i+1]], result+int(num[:i+1]))
            self.DFS(num[i+1:], target, ex+"-"+num[:i+1], stack+["-", num[:i+1]], result-int(num[:i+1]))
            ...
            if len(stack2) >=3:  # case of stack = ["1","+","2"]
                op, x = stack2[-2], int(stack2[-1])
                stack2[-1] = x*int(num[:i+1])
                if op == "+":
                    self.DFS(num[i+1:], target, ex+"*"+num[:i+1], stack2, result-x+x*int(num[:i+1]))
                elif op == "-":
                    self.DFS(num[i+1:], target, ex+"*"+num[:i+1], stack2, result+x-x*int(num[:i+1]))
