...
class Solution:
    def numberToWords(self, num: int) -> str:
        ...
        while len(num) > 3:
            ...
            if digit > 0:
                if int(num[:len(num)-digit*3]) > 0:   # only num > 0 to consider thousand, million, billon
                    ans += self.withinHundred(num[:len(num)-digit*3])+" "+digit3[digit]
                num = num[len(num)-digit*3:]
