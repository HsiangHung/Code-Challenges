...
class Solution:
    ...
    def findShortestWay(self, maze: List[List[int]], ball: List[int], hole: List[int]) -> str:
        ...
        while heap:
            ...
            if x2 != x and (x2, y) not in visited: heapq.heappush(heap, (step+dist, (x2, y), move+"l"))

            x2, y, dist = self.right(hole, x, y, maze)
            if x2 != x and (x2, y) not in visited: heapq.heappush(heap, (step+dist, (x2, y), move+"r"))
                
            x, y2, dist = self.up(hole, x, y, maze)
            if y2 != y and (x, y2) not in visited: heapq.heappush(heap, (step+dist, (x, y2), move+"u"))
            
            x, y2, dist = self.down(hole, x, y, maze)
            if y2 != y and (x, y2) not in visited: heapq.heappush(heap, (step+dist, (x, y2), move+"d"))
